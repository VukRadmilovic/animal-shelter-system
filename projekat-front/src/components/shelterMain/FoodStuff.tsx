...
export function FoodStuff({animals, shelter, shelterService} : FoodStuffProps) {
    ...
    const handleBuyFood = (animalType: string) => {
        ...
        if (portions && portions > 0) {
            shelterService.purchaseFood(animalType, portions)
                .then(response => {
                        ...
                        <Grid container key={index} sx={{display:'flex', flexDirection:'row', alignItems:'center'}}>
                            ...
                            <Grid item xs={2} sm={2} md={2} lg={2} xl={2} p={1}>
                                <TextField
                                    ...
                                    onChange={(e) => handlePortionChange(type, parseInt(e.target.value))}
                                />
                            </Grid>
                            <Grid item xs={2} sm={2} md={2} lg={2} xl={2} p={1}>
                                <Button type="button" variant="contained" color="primary" onClick={() => handleBuyFood(type)}>
                                    Buy food
                                </Button>
                            </Grid>
                        </Grid>
